...
x_train_d = reader.read_field('u_1')
...
test_out1  = x_test[:,:,:,T_in+1:T_out+1]
...
with torch.no_grad():
     for xx, yy, zz in test_loader_pred2:
        ...
        yy = yy.to(device)
        zz = zz.to(device)
        for t in range(0, T_out-T_in+45, step):
            z = zz[:,:,:,t:t+step]
            im = model2(xx,yy)         
            loss += torch.norm(im-z, p=2)/torch.norm(z, p=2)
            if t == 0:
...
#%%
scipy.io.savemat('pred/ns_wavelettransfomer_navierstokes_120_1.mat', mdict={'pred': pred0.cpu().numpy()})
# %%
...
#%%
torch.save(model2,'model/model_navierstokes_120_1')
...
for value in range(test_out1.shape[-1]):
    if value % 19 == 1:
        ...
        plt.imshow(test_out1[bt_no,:,:,value], cmap='jet', extent=[0,1,0,1], interpolation='Gaussian')
        ...
        plt.subplot(3,5, index+1+5)
        plt.imshow(pred0[bt_no,:,:,value], cmap='jet', extent=[0,1,0,1], interpolation='Gaussian')
        plt.colorbar(fraction=0.045)
        plt.xlabel('x',fontweight='bold'); plt.ylabel('y',fontweight='bold')
        plt.xticks(fontweight='bold'); plt.yticks(fontweight='bold');
